...
class AirCargoProblem(Problem):
            ...
            for a in self.airports:
                for c in self.cargos:
                    for p in self.planes:
                        ...
                        unload = Action(expr('Unload({}, {}, {})'.format(c, p, a)), [precond_pos, precond_neg],
                                      [effect_add, effect_rem])
    ...
    def result(self, state: str, action: Action):
        ...
        for fluent in old_state.pos:
            if fluent not in action.effect_rem: # persistence action(literal can persist if no action negates it)
                new_state.pos.append(fluent)
        for fluent in action.effect_add: #
            if fluent not in new_state.pos: # fluent(pos) been put into new_state_pos list after executing
                new_state.pos.append(fluent)
        for fluent in old_state.neg:
            if fluent not in action.effect_add: # persistence action
                new_state.neg.append(fluent)
        for fluent in action.effect_rem:
            if fluent not in new_state.neg: # fluent(neg) been put into new_state_neg list after executing
                new_state.neg.append(fluent)
