...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # PgNode_a has prenodes that are a subset of the previous S level will be add to a_levels
            if new_action_node.prenodes.issubset(self.s_levels[level]):
                ...
                # action's parent connect to s
                for s_node in self.s_levels[level]:
                    if s_node in new_action_node.prenodes:
                        s_node.children.add(new_action_node)
                        new_action_node.parents.add(s_node)
    ...
    def add_literal_level(self, level):
        ...
        pre_level = level - 1
        if pre_level >= 0:
            for a_node in self.a_levels[pre_level]:
                for eff_snode in a_node.effnodes:
                    # pre_action_level's effnodes add to S and build the requied connections
                    self.s_levels[level].add(eff_snode)
                    eff_snode.parents.add(a_node)
                    a_node.children.add(eff_snode)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        For example, Eat(Cake) and the persistence of Have(Cake)
        have inconsistent effects because they disagree on the effect Have(Cake).
        ...
        for eff_add1 in action1.effect_add:
            if eff_add1 in action2.effect_rem:
                return True
        for eff_rem1 in action1.effect_rem:
            if eff_rem1 in action2.effect_add:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # for eff_add1, eff_rem1, pre_pos1, pre_neg1 in \
        #         zip(action1.effect_add, action1.effect_rem, action1.precond_pos, action1.precond_neg):
        #     for eff_add2, eff_rem2, pre_pos2, pre_neg2 in \
        #             zip(action2.effect_add, action2.effect_rem, action2.precond_pos, action2.precond_neg):
        #         if eff_add1.__eq__(pre_neg2) or eff_rem1.__eq__(pre_pos2) or \
        #                 eff_add2.__eq__(pre_neg1) or eff_rem2.__eq__(pre_pos1):
        #             return True

        for eff_add1 in action1.effect_add:
            if eff_add1 in action2.precond_neg:
                return True
        for eff_rem1 in action1.effect_rem:
            if eff_rem1 in action2.precond_pos:
                return True
        for eff_add2 in action2.effect_add:
            if eff_add2 in action1.precond_neg:
                return True
        for eff_rem2 in action2.effect_rem:
            if eff_rem2 in action1.precond_pos:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for precon1, precon2 in zip(node_a1.parents, node_a2.parents):
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            ...
            for s_node in self.s_levels:
                if PgNode_s(goal, True) in s_node:
                    level_sum += process_level
                    break
                process_level += 1
